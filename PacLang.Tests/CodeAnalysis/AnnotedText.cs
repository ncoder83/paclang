    ...
    /*
            {
                ...
                x = 10
     */
    internal sealed class AnnotedText
    {
        public AnnotedText(string text, ImmutableArray<TextSpan> spans)
        {
            Text = text;
            Spans = spans;
        }
        ...
        public static AnnotedText Parse(string text)
        {
            text = Unindent(text);
            ...
            return new AnnotedText(textBuilder.ToString(), spanBuilder.ToImmutable());
        }

        private static string Unindent(string text)
        {
            var lines = UnindentedLines(text);

            return string.Join(Environment.NewLine, lines);
        }

        public static string[] UnindentedLines(string text)
        {
